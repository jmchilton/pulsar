...
``XXX_missing_file_max_retries`` limits retries after ``FileNotFoundError``.
It defaults to 5 and cannot extend ``XXX_max_retries``. Set it to 0 to use the
shared retry budget instead.
